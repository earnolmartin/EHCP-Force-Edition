...
function ext_init(){
    ...
    function rowContextMenu(grid, rowIndex, e, f) {
    	...
    	if(!gsm.isSelected(rowIndex)){
			gsm.selectRow(rowIndex);
		}
	...
	var viewport = new Ext.Viewport({
	    ...
	    items:[{
            ...
            region: "west",
        	id: "dirTree",
        	title: '<?php echo ext_Lang::msg('directory_tree', true ) ?> <img src="<?php echo _EXT_URL ?>/images/_reload.png" hspace="20" style="cursor:pointer;" title="reload" onclick="Ext.getCmp(\'dirTree\').getRootNode().reload();Ext.getCmp(\'dirTree\').getRootNode().select();chDir(\'\', false);" alt="Reload" align="middle" />', 
        	closable: false,
            width: 230,
            titlebar: true,
            autoScroll:true,
