	...
	*/
    function chDir( directory, loadGridOnly ) {
		...
		*/
		...
		var manipulateDirectory = directory;
		manipulateDirectory = manipulateDirectory.replace("ext_root", ""); 
		var dirs = manipulateDirectory.split('/');
		var fullNodeId = dirs.join('_RRR_');
		
		//alert(fullNodeId);

		// Select the root dir if we do not have a node id
		if(fullNodeId == ""){
			fullNodeId = "ext_root";
		}
		var node = Ext.getCmp("dirTree").getNodeById( fullNodeId );
		if(node != null){
			//alert('I am selecting a node');
			Ext.getCmp("dirTree").getSelectionModel().select(node);
		}
		reloadNode();
    }
	
	function reloadNode(){
		var selNode = Ext.getCmp("dirTree").getSelectionModel().getSelectedNode();
		if(selNode != null){
			setTimeout(function(){ selNode.reload(); }, 320);
		}
	}
...
function openActionDialog( caller, action ) {
	...
	switch( action ) {
		...
		case 'move':
			...
			if( action != "edit" ) {
            	dialog.on( 'hide', function() { dialog.destroy(true); reloadNode(); } );
            	dialog.show();
            }
            break;
		...
		case 'delete':
			...
			var filesAndDirsToDelete = "<br />";
			for(var i = 0; i < selectedRows.length; i++){
				var selectedRow = selectedRows[i];
				if(selectedRow !== null && typeof selectedRow === 'object'){
					filesAndDirsToDelete += "<br />File: " + selectedRows[i].data.name;
				}else{
					filesAndDirsToDelete += "<br /><b>Entire directory:</b> " + selectedRows[i];
				}
			}
			Ext.Msg.confirm('<?php echo ext_Lang::msg('dellink', true ) ?>?', String.format("<?php echo ext_Lang::err('miscdelitems', true ) ?>", num ) + filesAndDirsToDelete, deleteFiles);
			break;
		case 'extract':
