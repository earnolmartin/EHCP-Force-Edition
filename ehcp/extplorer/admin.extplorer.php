...
if(!isset($action) || empty($action)){
	unset($_SESSION['dirCurrentlyInPath']);
}
...
if(isset($_SESSION['dirCurrentlyInPath'])){
	$dir = $_SESSION['dirCurrentlyInPath'];
}
...
if( class_exists(strtolower($classname)) && is_callable(array($classname,'execaction'))) {
	...
	case "getdircontents":
			...
			$node = extGetParam( $_REQUEST, 'node' );
			$dirPost = extGetParam( $_REQUEST, 'dir' );
			
			if(empty($dirPost) && !empty($node)){
				$requestedDir = stripslashes(str_replace( '_RRR_', '/', $node));
			}
			
			if(empty($node) && !empty($dirPost)){
				$requestedDir = stripslashes(str_replace( 'ext_root', '', $dirPost));
			}
			
			//die($requestedDir . " is " . $dir);
			if( empty($requestedDir) || $requestedDir == 'ext_root') {
				$requestedDir = "";
			}
			$dir = $requestedDir;
			$_SESSION['dirCurrentlyInPath'] = $dir;
			//die($requestedDir . " is " . $dir);
