...
function apacheUseFPM(){
	...
	if [ "$distro" == "debian" ] && [ "$yrelease" -eq "9" ]; then
		repoOndrejExists=$(cat /etc/apt/sources.list.d/apache2.list | grep -o "packages.sury.org/apache2")
...
function disableRecursiveBIND(){ # by earnolmartin@gmail.com
	...
	if [ "$RESOLVCOUNT" == "1" ] && [ ! -z "$RESOLVLOCAL" ]; then
		...
		if [ -e "$bindOptionsFile" ]; then
			...
			if [[ "$distro" == "ubuntu" && "$yrelease" -le "18" ]] || [[ "$distro" == "debian" && "$yrelease" -le "11" ]]; then
...
function searchForServiceName(){	
	...
	if [ ! -z "$1" ]; then
		serviceName=$(ls /etc/init.d 2>/dev/null | grep -F -- "${1}" | sort -r | head -n 1)
		if [ ! -z "$serviceName" ]; then
			echo "${serviceName}" > "${serviceNameTempFile}"
		fi
		
		serviceName=$(find /lib/systemd/system -name "*${1}*" -exec basename {} .service \; 2>/dev/null | sort -r | head -n 1)
		if [ ! -z "$serviceName" ]; then
			echo "${serviceName}" > "${serviceNameTempFile}"
		fi
	
		serviceName=$(find /etc/systemd/system -name "*${1}*" -exec basename {} .service \; 2>/dev/null | sort -r | head -n 1)
		if [ ! -z "$serviceName" ]; then
			echo "${serviceName}" > "${serviceNameTempFile}"
		fi
