...
function runOp($op){ # these are like url to function mappers...  maps op variable to some functions in ehcp; This also can be seen as a controller in MVC model. 
	...
	switch ($op) {
		...
		default							: return $this->errorText("Internal EHCP Error - Undefined Operation: $op <br> This feature may not be complete.");break;
...
function activate(){
	...
	if(!$panelusername){
		...
		$this->output.="An activation code was sent to your email address of $email. Please insert this code now:<br>".inputform5(array(
																																	array('panelusername', 'lefttext'=>'Panel Username:'),
...
function addRedirect(){
	...
	if(!$_insert){
		$this->output.="Redirect Domain:<br>Include http:// or https://.<br><h5 style='margin:0px;'>(custom apache/nginx templates will be reset and lost for $domainname if redirected)</h5>".inputform5(array(
...
function infoemail($adminsubject,$adminmessage,$useremail,$usersubject,$usermessage,$todeveloper=True){
	...
	if(!$adminsubject) $adminsubject="Important";
	if(!$usersubject) $usersubject="Important";
...
function isuserpermited($permtype,$item='',$user='') {  # user permission check, is user authorized to do something
	...
	switch ($permtype) {
		case "deletedomain":
			...
			if(in_array($reseller, $userHasAccessToTheseChildrenUsers) or $this->isadmin()) return True;
		...
		case "caneditpaneluser":
			...
			if($this->activeuser==$panelusername and !$this->isadmin()) return $this->errorText('You are unable to edit your own account.');
		...
		case "domainowner":
			...
			if($this->isadmin() or trim($domainowner[$this->conf['domainstable']['ownerfield']]) == trim($this->activeuser) or trim($domainowner['reseller']) == trim($this->activeuser)) return True;
		break;

		default: return $this->errorText("Internal EHCP Error: Undefined permtype: $permtype");
	}
...
// No longer used 
// Domains are sorted alphabetically now...
/* 
function domainsettings(){
	...
	if($_insert){
		...
		$this->ok_err_text($success,"Successfully saved domain settings.","Failed to save domain settings.");
	} else {
...
function existscontrol($controls){
	...
	foreach($controls as $key=>$val){
		...
		switch($key){
			...
			default: return $this->errorText("Internal EHCP Error: Undefined parameter: $key=>$val  ar: ".print_r2($controls));
		}
...
function errorText($str='',$emailtoadmin=false){
	...
	if($this->conn->ErrorMsg()<>'') $str.="<hr>Last Database Error: ".$this->conn->ErrorMsg();
	...
	if($emailtoadmin) $this->infotoadminemail($str,"An Error has Occurred",false);
...
function afterInputControls($op='',$params=''){
	...
	if($op=='adddomain' or $op=='adddomaintothispaneluser'){ # common controls for both operation
		if($domainname=='') return $this->errorText("Domain name cannot be empty!");
		if($domainname==$this->miscconfig['dnsip'] or $domainname==$this->miscconfig['localip']) return $this->errorText("You cannot use the IP address of the server as a domain name: $domainname<br>DNS IP Address: ".$this->miscconfig['dnsip']."<br>Local IP: ".$this->miscconfig['localip']); # domainde kisitlama yok, ama boyle olunca, abuk subuk seyler olabilir.. ip girince de, sunucu paneli webden ulasilamaz oluyor..
	}
...
function addPasswordProtectedDir(){ #coded by earnolmartin@gmail.com
...
function addRemoteBackup(){ # coded by earnolmartin@gmail.com
...
function removeRemoteBackup(){ # coded by earnolmartin@gmail.com
...
// Update theme color
function updateThemeColor(){ # coded by earnolmartin@gmail.com
...
function updateThemeContrast(){ # coded by earnolmartin@gmail.com
	// This is an ajax call
...
// Generic cronjobs:
function addCronjob(){ # coded by earnolmartin@gmail.com
...
function removeCronjob(){ # coded by earnolmartin@gmail.com
...
function check_remote_mysql_connection($dbhost){
	if($this->connected_mysql_servers[$dbhost]) return $this->connected_mysql_servers[$dbhost];
	#$conn=$this->
...
function multiserver_add_domain_direct($named_params){
	...
	if($success){
		...
		$this->infotoadminemail($msg,$subj);
		if($email<>'') $this->sendEmail($email, $subj, $msguser);

		return $this->ok_err_text($success,"Domain $domainname was successfully added to the panel.",
...
function addDomainDirectToThisPaneluser($domainname,$ftpusername,$sync=True){
	...
	*/
		...
		if($email<>'') $this->sendEmail($email, "Domain Added to Panel", $msguser);
...
function deleteDomain(){
	...
	if($domainpaneluser==$this->activeuser) $panelusercount=2; # dont delete your self acount...
...
function addScript(){
	...
	if ((strpos($scriptname,"ehcp itself")!==false)or(strpos($scriptname,"ehcp webmail")!==false)) {				
		if(!$this->isadmin()) $this->errorTextExit("Only admins can install EHCP.  You should be able to access webmail at: http://webmail.yourdomain.com");
...
function addPanelUser(){
	...
	if(!$panelusername){
		...
		if(!$this->afterInputControls("addpaneluser",array('panelusername'=>$panelusername, 'email'=>$email))) return false;
...
function addPanelUserWithHostingPlan(){
	...
	if(count($rs) > 0){
		...
		if(!$_insert) {
			...
			if(!$this->afterInputControls("addpaneluser",array('panelusername'=>$client_panelusername, 'email'=>$client_email))) return false;
			$this->output.="Adding user:<br>";
...
function addFtpUserDirect($panelusername,$ftpusername,$password,$home,$upload,$download,$quota,$domainname='',$type='',$isSpecialHome=false,$status=''){
	...
	if(strstr($home,$this->ehcpdir)!==false) return $this->errorText($this->ehcpdir." location cannot be used for security purposes!");
...
function addSubDomain(){
	...
	if($subdomain){
		...
		if(!$this->beforeInputControls("addsubdomain",array())) return false;
...
function showSimilarFunctions($func){
	...
	switch($func){
		case 'ftp': 
			...
			break;
		case 'easyinstall' : $out="<a href='?op=addscript'>Easy Install More Packages</a>";break;
		case 'mysql' : $out="<a href='?op=domainop&amp;action=listdb'>List / Delete Mysql Databases</a><a href='?op=addmysqldb'>Add Mysql Database &amp; User</a><a href='?op=addmysqldbtouser'>Create MySQL Database and Assign to Existing Database User</a><a href='?op=dbadduser'>Add MySQL User to Existing Database</a><a href='/phpmyadmin' target=_blank>phpMyAdmin</a>";break;
		...
		case 'domain': $out="<a href='?op=addDomainToThisPaneluser'>Add Domain To My Account</a><a href='?op=adddomaineasy'>Easy Add Domain (with separate ftpuser)</a><a href='?op=adddomain'>Normal Add Domain (Separate ftp&panel user)</a><a href='?op=bulkadddomain'>Bulk Add Domain</a><a href='?op=adddnsonlydomain'>Add DNS-Only Hosting</a><a href='?op=adddnsonlydomainwithpaneluser'>Add DNS-Only Hosting with Separate Paneluser</a><a href='?op=addslavedns'>Make Domain a DNS Slave</a><a href='?op=removeslavedns'>Remove DNS Slave</a><a href='?op=adddomaineasyip'>Easy Add Domain to Different IP</a><a href='?op=setactiveserverip'>Set Active Webserver IP</a><a href='?op=listselectdomain'>List Domains</a>";break;
	...
	";break;
		...
		case 'vps': $out="<a href='?op=vps'>VPS Home</a><a href='?op=add_vps'>Add New VPS</a><a href='?op=settings&group=vps'>VPS Settings</a><a href='?op=vps&op2=other'>Other VPS Options</a>";break;
		...
		case 'global_templates': $out = "<a href='?op=manageglobalwebtemplates'>Manage Global Webserver Templates</a>"; break;
...
function addFtpToThisPaneluser(){ # add an ftp user freely under your master ftp directory
	...
	if($_insert){
		...
		if($success) {
			...
			if($email<>'') $this->sendEmail($email,"FTP Account Created", $msguser);
		}
...
function addSubDirectoryWithFtp(){
	...
	if($subdirectory){
		...
		if($success) {
			...
			if($email<>'') $this->sendEmail($email,"Subdirectory with FTP Account Created", $msguser);
		}
	...
	if($subdomain){
		...
		if($subdomain){
			...
			if($success) {
				...
				if($email<>'') $this->sendEmail($email,"Subdomain and FTP Account Created", $msguser);
			}
		...
		if(!$this->beforeInputControls('addftpuser')) return false;
		if(!$this->beforeInputControls('addsubdomainwithftp')) return false;
		$inputparams=array(
...
function subDomains(){
	...
	if($this->selecteddomain<>'') $filter.=" and domainname='$this->selecteddomain' ORDER BY subdomain ASC";
...
function editFtpUser(){
	...
	if(!$_insert){
		...
		if($newpass=='' and $newpass2=='') {
			...
			if($newpass<>$newpass2) $success=$this->errorText('The entered passwords do not match. Please try again.');
...
function setLanguage($lang,$quite=false){
	...
	if(!file_exists($file)) $this->errorTextExit("The selected localization file $file does not exist within this theme!  Try another theme.");
...
function securedelete($files,$serverip){
	...
	if(strpos($files,'..')!==false) return $this->errorText("Deleting the following files is forbidden: ($files)",True);
...
function runOpWrapper($op){ // normal operations
...
function runOp2Wrapper($op){ // for daemon ops
	$this->requireCommandLine(__FUNCTION__); // run from commandline.
...
<br /></b></font>";
		if(!$this->isloggedin) $this->infotoadminemail('Hi EHCP Admin,<br><br>The EHCP Daemon is not running!<br>'.$this->clientip,'EHCP Daemon Not Running On '.$this->clientip); # with if, reduce mail traffic..
		return false;
	}
}
...
function userop(){
	...
	switch($action){
		...
		default: return $this->errorText("userop: No action was provided.");
	}
}

function getMySqlDBAndUserInfo($id){ # by earnolmartin@gmail.com
...
function addMysqlDbDirect($myserver, $domainname, $dbusername, $dbuserpass, $dbuserhost, $dbname, $adduser = true, $allowRemoteAccess = false){
	...
	if($s===false) return $this->errorText("Error creating database ". $this->getDBError() ."<br>");
	...
	if($success and $myserver['host']<>'localhost'){ # if remote
		...
		if($success) $success=$success && mysqli_select_db($link, "ehcp") && mysqli_query($link, $q);
		if(!$success) return $this->errorText("MySQL Error ". $this->getDBError() ."<br>");
	}
	return $success;
}
...
function sifreHatirlat(){ # password reminder
	...
	if($email<>"") {
		...
		if($kayitliemail<>''){
			...
			$sayi=$this->recordcount("hash",$filt3);
			if($sayi==0) $this->errorTextExit("Received invalid password reset information. Verify the password reset URL that was sent to your email.");
